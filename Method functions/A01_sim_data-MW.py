...
def simulation_01(seed,num_firing,num_nonfire,effect=0.5,n0=30,n1=30,threshold=0.05,show_plot=False,s0=1,s1=1):
    ...
    # Calculate Mann-Whitney U test statistics and p-values
    p_values = []
    for i in range(num_firing):
        statistic, p_value = _mannwhitneyu(control_group[:, i], treatment_group[:, i])
    ...
    plt.title('Distribution of p-values (Mann-Whitney U test)')
    plt.show()
