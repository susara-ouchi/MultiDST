...
for i in range(num_iter):
    if not p_values or not fire_index or not nonfire_index:
        print("One or more input lists is empty. Cannot generate histogram.")
    else:
        plot1 = fire_hist(p_values, fire_index, nonfire_index,title="Histogram of p-values - Before weighting",col1 = 'skyblue', col2 = 'greenyellow')
    ...
    plot2 = plot_heatmap(methods, sig_indices, title=f"Significant index plot - Before Weighting")
    ...
    if i==0:
        ...
        p_values = p_values2w
        fire_index = fire_index2w
        nonfire_index = nonfire_index2w 
        

    l0,l1,l2,l3,l4,l5,l6,l7 = common_indices(p_values, sig_bonf_p, sig_holm_p, sig_sgof_p, sig_bh_p, sig_by_p, sig_q)
    max_weight = 1.4

    weighted_p = weighted_p_list(p_values,l0,l1,l2,l3,l4,l5,l6, weights="multi", max_weight = max_weight)
    ...
    if not p_values or not fire_index or not nonfire_index:
        ...
        plot3 = fire_hist(p_values, fire_index, nonfire_index, title="Histogram of p-values - After weighting", col1='skyblue', col2='greenyellow')
    results = DSTmulti_testing(p_values, alpha=0.05, weights=False)
    print(results)   
    ...
    plot4 = plot_heatmap(methods, sig_indices, title=f"Significant index plot after weighting")
    ...
    p_index2w  = [i for i, val in enumerate(og_p_values) if i not in rejections]
    p_values2w = [val for i, val in enumerate(og_p_values) if i not in rejections]
    fire_index2w = [i for i,val in enumerate(p_values2) if p_values2[i] in p_value_fire]
    nonfire_index2w = [i for i,val in enumerate(p_values2) if p_values2[i] in p_value_nonfire]
    ...
    df_sigp[['Bonferroni', 'Holm', 'SGoF', 'BH', 'BY', 'Q-value']]
